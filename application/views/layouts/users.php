...
<html lang="en" class="app">
  <head>
    ...
    <script src="<?=base_url()?>resource/js/webcam/webcam.js" ></script>
    <script src="<?=base_url()?>resource/rcswitcher-master/js/rcswitcher.js" ></script>
    ...
    <script src="<?=base_url()?>resource/plugin/counterup/jquery.waypoints.min.js"></script>
    <script src="<?=base_url()?>resource/plugin/counterup/jquery.counterup.min.js"></script>
...
function edit_usermenu(id)
{
      ...
      $.ajax({
        ...
        success: function(data)
        {
            ...
            $('[name="verified_second"]').val(data.Verified_Second);
...
function addRow_DeliveryOrder(){
    ...
    detailrow=detailrow+"<td><div class='col-md-12' style='padding:0px'>";
...
function edit_document(row_id){
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            if(data.komisi_supir == 0){
                $.ajax({
                    type: "POST",
                    url : "<?php echo base_url('finances/getAmountCost'); ?>",
        			data: "from_id="+data.destination_from_rowID+'&to_id='+data.destination_to_rowID+'&jo_type='+data.jo_type+
                        '&vehicle_type='+data.vehicle_type_rowID+'&<?=$this->security->get_csrf_token_name()?>='+'<?=$this->security->get_csrf_hash()?>',
                    cache:false,
                    dataType:"JSON",
                    success: function(data_fare_trip){
                        if(data_fare_trip == null){
                            $('#komisi_supir').val(number_format(data.komisi_supir,0,',','.','format'));
                            $('#komisi_kernet').val(number_format(data.komisi_kernet,0,',','.','format'));
                        }
                        else{
                            $('#komisi_supir').val(number_format(data_fare_trip.komisi_supir,0,',','.','format'));
                            $('#komisi_kernet').val(number_format(data_fare_trip.komisi_kernet,0,',','.','format'));            
                        }                      
                    },
        			error: function(xhr, status, error) {
        				document.write(xhr.responseText);
...
function add_ap(){
    ...
    $('#modal_form').modal('show');
    <?php
    if($this->uri->segment(1) == "account_payable"){
    ?>
        $('.modal-title').text('Add Account Payable (AP)');
    <?php
    }
    else if($this->uri->segment(1) == "kontra_bon"){
    ?>
        $('.modal-title').text('Add Kontra Bon (KB)');
    <?php
    }
    ?>
    //$('#ap_type').select2();
...
function set_due_date_kb(){
    $.ajax({
        type: "POST",
        url : "<?php echo base_url('kontra_bon/get_data_due_date'); ?>",
		data: "top="+$('#top').val()+'&<?=$this->security->get_csrf_token_name()?>='+'<?=$this->security->get_csrf_hash()?>',
        cache:false,
        success: function(data){
            $('#come_back').val(data);
        },
                ...
                if (r){
                    if(clickSave == 0){
                        ...
                        $.ajax({
                            ...
                            ?>
                                url : "<?php echo base_url('kontra_bon/save_ap')?>",
                            <?php
                            }
                            ?>
                            ...
                            {                 	            
                                if (result.success){ 
                                    ...
                                    ?>
                                        location.replace("<?php echo base_url('kontra_bon')?>");
                                    <?php
                                    }
                                    ?>
...
function edit_ap(trx_no){
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            ?>
                $('.modal-title').text('Update Kontra Bon (KB)');
            <?php
            }
            ?>         
...
$('#ap_type').change(function(){
    ...
    $.ajax({
        ...
        success: function(data){
            <?php
            if($this->uri->segment(1) == 'account_payable'){
                echo "$('#creditor_id').html(data);
                      $('#creditor_id').select2('val','');";
            }
            else{
                echo "$('#creditor_list').html(data);";
            }
            ?>
            ...
            for(z=1;z<=looprows; z++){  
                if(document.getElementById('debtor_creditor_rowID_'+z)!=null  ){
                    if(document.getElementById('debtor_creditor_type_'+z).value == "C" ){
                        $('#debtor_creditor_rowID_'+z).select2();
                        document.getElementById("debtor_creditor_rowID_"+z).innerHTML=document.getElementById("creditor_list").innerHTML;
                        $('#debtor_creditor_rowID_'+z).select2('val','');
                    }
                }
            }
...
function addNewRow_DeliveryOrder_AP(){
    ...
    detailrow=detailrow+"<td >";
    var text1="<input class='form-control' id='do_jo_no"+totrowDO+"' name='detailDO["+totrowDO+"][do_jo_no]' type='text' style='text-align:right;height:30px;width:130px;background-color:#fff;cursor: pointer;' onclick='selectJO_AP_Detail("+totrowDO+")' readonly='' value='' /><input id='jo_year"+totrowDO+"' name='detailDO["+totrowDO+"][jo_year]' type='hidden' value=''  /><input id='jo_month"+totrowDO+"' name='detailDO["+totrowDO+"][jo_month]' type='hidden' value=''  /><input id='jo_code"+totrowDO+"' name='detailDO["+totrowDO+"][jo_code]' type='hidden' value=''  /><input id='jo_fare_trip_id"+totrowDO+"' name='detailDO["+totrowDO+"][jo_fare_trip_id]' type='hidden' value=''  />";
    detailrow=detailrow+text1;
    detailrow=detailrow+"<input type='hidden' class='form-control' name='detailDO["+totrowDO+"][komisi_supir]' id='komisi_supir_"+totrowDO+"' ><input type='hidden' class='form-control' name='detailDO["+totrowDO+"][komisi_kernet]' id='komisi_kernet_"+totrowDO+"' ><input type='hidden' class='form-control' name='detailDO["+totrowDO+"][deposit]' id='deposit_"+totrowDO+"' ><input type='hidden' class='form-control' name='detailDO["+totrowDO+"][amount]' id='amount_"+totrowDO+"' ></td>";
    ...
    detailrow=detailrow+"<td><div class='col-md-12' style='padding:0px 15px 0px 0px'>";
    ...
    detailrow=detailrow+"<td >";
    var text4="<input class='form-control' id='amount_ap"+totrowDO+"' name='detailDO["+totrowDO+"][amount_ap]'  type='text' style='text-align:right;height:30px;width:120px;background-color:white;border:solid 1px #ccc;'  value='0' onkeyup='sumAmountAP()' readonly /> <input type='hidden' id='wholesale"+totrowDO+"' name='detailDO["+totrowDO+"][wholesale]' /> <input type='hidden' id='price_amount"+totrowDO+"' name='detailDO["+totrowDO+"][price_amount]' />  <input type='hidden' id='price_20ft"+totrowDO+"' name='detailDO["+totrowDO+"][price_20ft]' />  <input type='hidden' id='price_40ft"+totrowDO+"' name='detailDO["+totrowDO+"][price_40ft]' />  <input type='hidden' id='price_45ft"+totrowDO+"' name='detailDO["+totrowDO+"][price_45ft]' />";
    detailrow=detailrow+text4;
    detailrow=detailrow+"</td>";
	...
	success: function(result){
		...
		$.each(result, function(key, val) {	
			...
			$.ajax({
				type: "POST",
				url : "<?php echo base_url('account_payable/get_data_jo'); ?>",
				data: "jo_no="+val.jo_no+'&<?=$this->security->get_csrf_token_name()?>='+'<?=$this->security->get_csrf_hash()?>',
				cache:false,
				dataType: 'json',
				success: function(data_jo){
					$('#jo_year').val(data_jo.year);   // JO Year
...
function selectJO_AP_Detail(row_id){
    if($('#row_id').val() == '')
        bersihDO_AP();
    ...
    var table = $('#tbl-joborder').DataTable({
     ...
     $('#tbl-joborder tbody').on('click', 'tr', function () {
        ...
        // Isi value detail DO
        $('#do_jo_no'+row_id).val($('#do_jo_no').val());  // No JO
        $('#jo_year'+row_id).val($('#jo_year').val());   // JO Year
        $('#jo_month'+row_id).val($('#jo_month').val());  // JO Month
        $('#jo_code'+row_id).val($('#jo_code').val());
        $('#jo_fare_trip_id'+row_id).val($('#jo_fare_trip_id').val());
        $('#price_amount'+row_id).val($('#price_amount').val());
        $('#wholesale'+row_id).val($('#wholesale').val());
        $('#price_20ft'+row_id).val($('#price_20ft').val());
        $('#price_40ft'+row_id).val($('#price_40ft').val());
        $('#price_45ft'+row_id).val($('#price_45ft').val());
        
        if($('#jo_type').val() == '1'){
            $('#ContType'+row_id).val('');    
            $('#container_no'+row_id).val('');                                    
    
            $('#ContType'+row_id).prop('disabled',true);    
            $('#container_no'+row_id).prop('disabled',true);                                    
        }
        else{
            $('#ContType'+row_id).val('');    
            $('#container_no'+row_id).val('');                                    
    
            $('#ContType'+row_id).prop('disabled',false);    
            $('#container_no'+row_id).prop('disabled',false);        
        }
        
        if($('#jo_type').val() == '1' || $('#jo_type').val() == '3'){
            if($('#wholesale'+row_id).val() == '1'){
                $('#amount_ap'+row_id).val(tandaPemisahTitik($('#price_amount'+row_id).val()));
            }
            else{
                $('#received_weight'+row_id).attr('onkeyup','IsNumericOnly(this);setPriceAmount('+row_id+')');
                $('#received_weight'+row_id).attr('onfocus','setPriceAmount('+row_id+')');
                $('#amount_ap'+row_id).val(tandaPemisahTitik($('#price_amount'+row_id).val()));
            }
        }
        else{ // Container
            if($('#wholesale'+row_id).val() == '1'){
                ...
                $('#received_weight'+row_id).attr('onkeyup','IsNumericOnly(this);');
                $('#received_weight'+row_id).attr('onfocus','');
                $('#amount_ap'+row_id).val(tandaPemisahTitik($('#price_20ft'+row_id).val()));
                $('#ContType'+row_id).val('20');
                $('#ContType'+row_id).attr('onchange','setContainerPriceAmount('+row_id+')');
                
            }
        }
        
        $('#received_weight'+row_id).focus();
...
function get_data_reference_kb(reference_no,reference_date,reference_debtor_creditor_id,reference_desc,gl_type,tax,amount){
    $('#reference_no').val(reference_no);
    $('#reference_date').val(reference_date);
    $('#reference_debtor_creditor_id').val(reference_debtor_creditor_id);
    $('.gl_remark').val(reference_desc);
    ...
    $.ajax({
        ...
        success: function(data){
            if(data.count_detail > 0){
                ...
                $('#cash_bank_'+totrowGl).select2('val','');
                $('#debtor_creditor_type_'+totrowGl).val('C').change();
                $('#debtor_creditor_rowID_'+totrowGl).select2('val',reference_debtor_creditor_id);
                $('#descs_'+totrowGl).val('');
                ...
                if(tax > 0){
                    ...
                    $('#cash_bank_'+totrowGl).select2('val','46');
                    $('#debtor_creditor_type_'+totrowGl).val('C').change();
                    $('#debtor_creditor_rowID_'+totrowGl).select2('val',reference_debtor_creditor_id);
                    $('#descs_'+totrowGl).val('');

                    totalDebit += parseFloat(tax);
                    $('#amount_debit_'+totrowGl).val(number_format(tax,2,',','.','format'));                    
                ...
                // Credit
                add_gl_detail(); 
                $('#cash_bank_'+totrowGl).select2('val','83');
                $('#debtor_creditor_type_'+totrowGl).val('C').change();
                $('#debtor_creditor_rowID_'+totrowGl).select2('val',reference_debtor_creditor_id);
                $('#descs_'+totrowGl).val(reference_desc);
                
                var credit_amt = parseFloat(tax) + parseFloat(amount);
                totalCredit += parseFloat(credit_amt);
                $('#amount_credit_'+totrowGl).val(number_format(credit_amt,2,',','.','format'));                                
...
function get_data_reference_bank(reference_no,reference_date,reference_debtor_creditor_id,reference_desc,gl_type,amount){
    $('#reference_no').val(reference_no);
    $('#reference_date').val(reference_date);
    $('#reference_debtor_creditor_id').val(reference_debtor_creditor_id);
    $('.gl_remark').val(reference_desc);
...
function click_verify_gl(){
    if($('#verified').is(':checked',true)) {
        $('#btnSave').hide();
        $('#btnVerify').show();
    }
    else{
    ...
    $('#verified_status_second').val($('#verified_status').val());
        
}

function show_modal_gl_verify(){
    $('#modal_verifikasi').modal('show');
}

function gl_verify_password(){
    $.ajax({
        url : "<?php echo base_url('general_ledger/verify_password')?>",
        type: "POST",
        data: $('#form_verify').serializeArray(),
        dataType: "JSON",
        success: function(result)
...
function save_general_ledger(){
    ...
    var reference_no = $('#reference_no').val();
    var gl_remark = $('#gl_remark').val();
    var trx_amt = parseFloat(number_format($('#trx_amt').val(),0,',','.','deformat'));
    ...
    if(totNil_d != trx_amt){
        balance_trx = cekValidasi(balance_val,'General Ledger','is Unbalanced');   
    }
    
    validasi = data1+data2+data3+data4+gl_detail+balance+balance_trx;
                    ...
                    $.ajax({
                        ...
                        success: function(result)
                        {
                            if (result.success){ 
                                ...
                                //sweetAlert('<?=lang('information')?>',''+result.msg);   
                                location.replace("<?=base_url('journal')?>");
...
function edit_general_ledger(gl_no){
    $.ajax({
        ...
        success: function(data)
        {
            ...
            if(data.verified == 1){
                $('.verified_second').html('Second');
            }
            else{
                $('.verified_second').html('');
            }
            
            $('#rowID').val(data.rowID);
            $('#journal_no').val(data.journal_no);
            $('#trx_amt').val(number_format(data.trx_amt,0,',','.','format'));
            $('#gl_date').val(toMmDdYy(data.journal_date));
            $('#gl_type').val(data.journal_type).change();
            $('#ap_type').val(data.creditor_type_rowID).change();
            $('#reference_no').val(data.ref_no);
            $('#reference_date').val(data.ref_date);
            $('#gl_remark').val(descs);
            $('#verified_status').val(data.verified);
            $('#user_verified').val(data.user_verified);
            $('#date_verified').val(data.date_verified);
            $('#time_verified').val(data.time_verified);
            $('#user_created').val(data.user_created);
            $('#date_created').val(data.date_created);
            $('#time_created').val(data.time_created);
            ...
            showDetailDeliveryOrder(data.ref_no);    
            
            $('#gl_date').attr('readonly',true);
            
            <?php
            if($this->user_profile->get_user_access('Verified') == 1){
                echo "$('.verify_status').show()";
            }
            else{
                echo "$('.verify_status').hide()";
            }
            ?>
...
function verified_general_ledger(gl_no){
    swal({
      title: "Are you sure?",
      text: "verify this data!",
      type: "warning",
    ...
    function(){
        $.ajax({
            url:'<?php echo base_url(); ?>general_ledger/verify_data',
            type: "POST",
            data: "gl_no="+gl_no+'&<?=$this->security->get_csrf_token_name()?>='+'<?=$this->security->get_csrf_hash()?>',
            dataType: "JSON",
            success: function(data)
            {
                swal("Verify!", "<?=lang('verify_succesfully') ?>.", "success");
                location.replace("<?php echo base_url()?>general_ledger");
            },
            error: function (jqXHR, textStatus, errorThrown)
            {
...
function print_general_ledger(trx_no){
    window.open('<?php echo base_url()?>general_ledger/print_general_ledger/'+trx_no);
}
    ...
    $.ajax({
		...
		success: function(result){
    		...
    		$.each(result, function(key, val) {	
                ...
                $('#debtor_creditor_rowID_'+x).select2('val','');
