...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $department, $login_by_username, $login_by_email)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->users->$get_user_func($login, $department))) {	// login ok
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
                    ...
                    /*else if ($department <> $user->dep_rowID) {									// fail - banned
						$this->error = array('department' => 'auth_incorrect_department');
					}
                    */ 
                    else {
						$this->ci->session->set_userdata(array(
								...
								'user_id'	=> $user->rowID,								
								'company_rowID'	=> $user->company_rowID,
								'dep_rowID'	=> $user->dep_rowID,
								'partial_data'	=> $user->partial_data,
								'role_rowID'	=> 4,
								'role_rowID_verify'	=> $user->role_rowID,
								'status'	=> ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED,
								'comment_created'	=> $user->comment_created,
								'comment_updated'	=> $user->comment_updated,
								'comment_deleted'	=> $user->comment_deleted,
								'comment_viewed'	=> $user->comment_viewed,
								'comment_activated'	=>$user->comment_activated,
								'comment_replied'	=> $user->comment_replied,
								'ticket_created'	=> $user->ticket_created,
								'ticket_updated'	=> $user->ticket_updated,
								'ticket_deleted'	=> $user->ticket_deleted,
								'ticket_viewed'		=> $user->ticket_viewed,
								'ticket_replied'	=> $user->ticket_replied,
								
						));
						
						$this->_log_activity('Login '.$user->rowID.'-'.ucwords($user->username),$icon = 'fa-sign-in'); //log activity
...
/* 							if ($remember) {
	...
	function logout()
	{
		...
		$this->_log_activity('Logout ' . $this->ci->session->userdata('user_rowID') . '-' . ucwords($this->ci->session->userdata('username')),$icon = 'fa-sign-out'); //log activity
	...
	{
		return $this->ci->session->userdata('user_rowID');
	}

	function get_role_id()
	{
		return $this->ci->session->userdata('role_rowID');
	}
	...
	{
		$this->ci->db->where('rowID',$role);
		$this->ci->db->select('role');
		$query = $this->ci->db->get('sa_roles');
			if ($query->num_rows() > 0)
				{
			 $row = $query->row();
			 return $row->role;
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $fullname, $company, $departement, $role, $phone, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			...
			$data = array(
				...
				'company_rowID'	=> $this->ci->session->userdata('company_rowID'),
				'dep_rowID'	=> $departement,
				'email'		=> $email,
				'role_rowID' => $role,
	...
	private function _log_activity($activity,$icon){
			$this->ci->db->set('module', 'Login');
			$this->ci->db->set('user_rowID', $this->get_user_id());
			$this->ci->db->set('activity', $activity);
			$this->ci->db->set('icon', $icon);
			$this->ci->db->insert('activities'); 
	}
}
