...
class Realizations_branch_model extends CI_Model
{
    ...
    function simpan_data_do($sa_spec_prefix, $alloc_code, $alloc_no, $dataPost, $detDO = array(), $get_data_header)
    {
        $count_container = 0;
        $container_size = '';
    ...
    function get_all_records_list($partial_data, $dep_rowID, $start_date, $end_date)
    {
        ...
        $this->db->join('sa_users AS e', 'e.rowID=a.user_created', 'LEFT');
        $this->db->where('a.deleted', 0);
        $this->db->where('b.deleted', 0);
        $this->db->where('e.dep_rowID', $this->session->userdata('dep_rowID'));
