...
class cash_advance_type_model extends CI_Model
{
    ...
    function cash_advance_type_details($cash_advance_type)
    {
        $query = $this->db->where('rowID', $cash_advance_type)->get('sa_advance_type');
        if ($query->num_rows() > 0) {
            return $query->result();
        }
    }
