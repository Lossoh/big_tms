...
class View extends MX_Controller {
	...
	{		
		...
		$this->template->title(lang('ticket_respond').' - '.$this->config->item('comp_name'). ' '. $this->config->item('version'));		
		$data['page'] = lang('ticket_respond');		
		$data['event_lists'] = $this->comments->get_all_records($table = 'lg_comments', $array = array('deleted' => 0, 'activated' => 1),$join_table = '',$join_criteria = '','case_description');
		$data['event'] = $this->uri->segment(4);	
		$data['event_replies'] = $this->comments->case_comments($this->uri->segment(4));
		$data['active'] = $this->comments->get_value($table = 'lg_comments',
			$array = array('rowID' => $this->uri->segment(4) ),'activated');
		$data['comment_replied'] = $this->session->userdata('comment_replied');
		
		$this->template
		->set_layout('users')
		->build('comments_details',isset($data) ? $data : NULL);
	}		
	...
	function add()
	{
		if ($this->input->post()) {
			...
			$this->load->view('modal/add_ticket');
		}
	}
