...
class Truck_model extends CI_Model
{
	...
	function truck_details($truck_id)
	{
		
		$query = $this->db->query("SELECT transporter_id, truck_id, gps_id, truck_ref, truck_name, truck_type_id, No_Urut_Ref, Nm_Ref
		FROM fx_mst_trucks a 
		LEFT JOIN fx_mst_reference b ON b.No_Urut_Ref = a.truck_type_id AND b.Type_Ref='truck_type'
		WHERE a.truck_id=$truck_id AND a.deleted=0")->result();
