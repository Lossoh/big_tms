...
class Finances_model extends CI_Model
{
        ...
        $dataPost)
    {
        ...
        $cash_advance_data = array(
            ...
            'co_driver_rowID' => $dataPost['co_driver_rowID'],
            ...
            'split_status' => $dataPost['split_status'],
            'dep_rowID' => $this->session->userdata('dep_rowID'),
            'barcode_no' => strtoupper($dataPost['barcode_no']),
            'spk_item_rowID' => strtoupper($dataPost['spk_item_rowID']),
            'spk_container_no' => strtoupper($dataPost['spk_container_no']),
            ...
            'date_created' => $cash_advance_date,//date('Y-m-d'),
        ...
        $cash_gl_rowID, $advance_name, $debtor_name, $trx_no_cb, $dataPost)
    {
        ...
        $cb_trx_data = array(
            ...
            'cg_void_status' => 'A',
            'user_created' => $this->session->userdata('user_rowID'),
            'date_created' => $cash_advance_date,
            'time_created' => date('H:i:s'));
    ...
    function simpanCashBankHeaderSecond($sa_spec_prefix_cb, $alloc_code_cb, $cash_advance_no,
        $cash_gl_rowID, $advance_name, $debtor_name, $trx_no_cb, $dataPost)
    {
        $total_amount = $dataPost->os_amount;
        ...
        $cb_trx_data = array(
            ...
            'debtor_creditor_type' => 'D',
            'coa_rowID' => $cash_gl_rowID,
            'descs' => strtoupper($advance_name . ' TAHAP KEDUA A/N ' . $debtor_name),
            'trx_amt' => $total_amount * -1,
            'debtor_creditor_rowID' => $dataPost->employee_driver_rowID,
            'recon_status' => 'N',
            'recon_date' => '1901-01-01',
            'cg_void_status' => 'A',
            'user_created' => $this->session->userdata('user_rowID'),
    ...
    function simpanCashBankDetail($sa_spec_prefix_cb, $alloc_code_cb, $cash_advance_no,
        $cash_gl_rowID, $advance_name, $debtor_name, $trx_no_cb, $dataPost)
    {
        $total_amount = str_replace('.', '', $dataPost['amount']) + str_replace('.', '',$dataPost['extra_amount']);
        ...
        $data = array(
                'cb_trx_hdr_prefix' => $sa_spec_prefix_cb,
                'cb_trx_hdr_year' => $year,
                'cb_trx_hdr_month' => $month,
                'cb_trx_hdr_code'  => $alloc_code_cb,
                'trx_no' => $trx_no_cb,
                'row_no' => 1,     
                'payment_method' => 'cash',
    ...
    function simpanCashBankDetailSecond($sa_spec_prefix_cb, $alloc_code_cb, $cash_advance_no,
        $cash_gl_rowID, $advance_name, $debtor_name, $trx_no_cb, $dataPost)
    {
        $total_amount = $dataPost->os_amount;
            ...
            $debtor_name, $new_cash_advance_code, $dataPost)
    {
        ...
        $gl_trx_hdr_data = array(
            ...
            'trx_amt' => $total_amount,
            'ref_prefix' => $cash_out_prefix_cd,
            'ref_year' => $alloc_date_year,
            'ref_month' => $alloc_date_month,
            'ref_code' => $new_cash_advance_code,
            'ref_no' => $cash_advance_no,
            'ref_date' => $alloc_date,
            'user_created' => $this->session->userdata('user_rowID'),
    ...
    {   
        ...
        $gl_trx_dtl_d_data = array(
            ...
            'gl_trx_hdr_ref_prefix' => $cash_out_prefix_cd,
            'gl_trx_hdr_ref_year' => $alloc_date_year,
            'gl_trx_hdr_ref_month' => $alloc_date_month,
            'gl_trx_hdr_ref_code' => $new_cash_advance_code,
            'gl_trx_hdr_ref_no' => $cash_advance_no,
            'gl_trx_hdr_ref_date' => $alloc_date,
            'modul' => 'CB',
            'cash_flow' => 'Y',
            'base_amt' => 0,
            'tax_no' => '',
    ...
    function simpan_data_do($sa_spec_prefix, $alloc_code, $alloc_no, $dataPost, $detDO = array(), $condition_commission)
    {
    ...
    function update_cash_advance($alloc_no,$dataPost)
    {
        ...
        $this->db->set('date_modified', $alloc_date);
    ...
    function get_all_records_list($departement, $dep_rowID, $start_date, $end_date)
    {
        $str_partial_data = '';
        if ($departement)
        {
        ...
        $this->db->join('sa_koordinat_poi AS j', 'j.rowID=g.coordinate_rowID', 'LEFT');
        $this->db->join('sa_koordinat_poi AS k', 'k.rowID=h.coordinate_rowID', 'LEFT');
        ...
        //return $this->db->get()->result_array(); //if($departement>0){$this->db->where('i.dep_rowID =',$dep_rowID)};
        */
    }
    
    function get_all_records_loan_list($departement, $dep_rowID, $start_date, $end_date)
    {
        $str_partial_data = '';
        if ($departement)
        {
    ...
    function get_all_records_list_bonus_nol($departement, $dep_rowID, $start_date, $end_date)
    {
        $str_partial_data = '';
        if ($departement)
        {
    ...
    function get_all_records_has_not_cb($departement, $dep_rowID, $start_date, $end_date)
    {
        $str_partial_data = '';
        if ($departement)
        {
        ...
        $sql = "SELECT `a`.*, `b`.`by_jo` as by_jo, `b`.`only_driver` as only_driver, `b`.`fare_trip` as fare_trip, `b`.`advance_name`, `c`.`debtor_cd` as debtor_code, 
                ...
                FROM (`cb_cash_adv` AS a) LEFT JOIN `sa_advance_type` AS b ON `b`.`rowID`=`a`.`advance_type_rowID` 
											...
											LEFT JOIN `sa_koordinat_poi` AS j ON `j`.`rowID`=`g`.`coordinate_rowID` 
											LEFT JOIN `sa_koordinat_poi` AS k ON `k`.`rowID`=`h`.`coordinate_rowID`                 
                WHERE `a`.`deleted` = 0 AND a.trx_no != '' AND `a`.`advance_balance` < 0 AND a.advance_type_rowID != 4 ".$str_partial_data."
                ORDER BY `a`.`advance_date` desc";
    ...
    function get_all_records_list_by_id($row_id)
    {
        ...
        $this->db->join('sa_item AS k', 'k.rowID=a.spk_item_rowID', 'LEFT');
        $this->db->where('a.deleted =', 0);
	...
	{
	    $sql = "SELECT a.vehicle_id, `b`.`police_no`, b.vehicle_type, a.speed, SUBSTRING(TRIM(CONV(a.status,16,2)),-2) as status, (a.latitude*0.000001) as latitude, 
    ...
    function get_document_by_do_no($do_no)
    {
        $this->db->select('*');
        $this->db->from('tr_do_trx');
        $this->db->where('deleted', 0);
    ...
    function get_ca_fare_trip_by_advance_no($advance_no)
    {
        $this->db->select('a.*, b.os_amount');
        $this->db->from('cb_cash_adv as a');
        $this->db->join('sa_fare_trip_hdr AS b', 'b.rowID = a.fare_trip_rowID', 'LEFT');
        $this->db->where('a.deleted', 0);
    ...
    function get_cash_bank_by_advance_no($advance_no)
    {
        $this->db->select('*');
        $this->db->from('cb_trx_hdr');
        $this->db->where('deleted', 0);
        $this->db->where('advance_invoice_trx_no', $advance_no);
        return $this->db->get()->result();
    }
    ...
    function check_no_do_by_trx_no($trx_no){
        $sql = "SELECT * FROM tr_do_trx 
                WHERE deleted = 0 AND trx_no = '".$trx_no."' AND do_no = ''";
        $query = $this->db->query($sql);
    ...
    function updateQueue_by_debtor($debtor_id, $date, $user_id)
    {
        $this->db->set('date_modified', $date);
        $this->db->set('user_modified', $user_id);
    ...
    function get_data_cash_advance_jo_by_from_to_type($from_id,$to_id,$type)
    {
    ...
    function get_data_commission_by_ca_no($advance_no)
    {
        $this->db->select('a.*, b.period');
        $this->db->from('cb_cash_adv_alloc as a');
        $this->db->join('tr_commission_trx as b', 'a.commission_no = b.commission_no AND b.deleted = 0', 'left');
        $this->db->where('a.deleted', 0);
