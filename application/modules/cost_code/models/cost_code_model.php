...
class Cost_code_model extends CI_Model
{
    ...
    function get_all_record_data(){
   			...
   			$this->db->select("a.*,
                CONCAT(b.acc_cd,' - ',b.acc_name) AS wip_acc,
                concat(c.acc_cd,' - ',c.acc_name) as cogs_acc ",false);
			$this->db->from('sa_cost as a');
			$this->db->join('gl_coa as b', 'b.rowID = a.wip_acc_rowID','left');
            $this->db->join('gl_coa as c', 'c.rowID = a.cogs_acc_rowID','left');
    ...
    function get_pdf()
    {
        ...
        $this->db->select("a.*,
                CONCAT(b.acc_cd,' - ',b.acc_name) AS acc_wip,
                CONCAT(c.acc_cd,' - ',c.acc_name) AS acc_cogs", false);
        $this->db->from('sa_cost as a');
        $this->db->join('gl_coa as b', 'b.rowID = a.wip_acc_rowID', 'left');
        $this->db->join('gl_coa as c', 'c.rowID = a.cogs_acc_rowID', 'left');
        $this->db->where('a.deleted', 0);
