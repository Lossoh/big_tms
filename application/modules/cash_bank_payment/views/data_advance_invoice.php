...
?>
<table id="tbl_advance_invoice" class="table table-striped table-hover b-t b-light text-sm">
    <thead>
      <tr>
        <th><?=lang('no')?></th>
      ...
      if (!empty($get_data)) {
        ...
        foreach ($get_data as $cash_advance_list) { 
            ...
            if($payment_type == 'P'){
                if($total_balance < 0){
      ?>
                  <tr style="cursor: pointer;" onclick="get_data_invoice_ca('<?=$cash_advance_list->advance_no?>','<?=$cash_advance_list->advance_balance?>','<?=$description?>')">
            		<td><?=$no++?></td>
...
<table id="tbl_advance_invoice" class="table table-striped table-hover b-t b-light text-sm">
	...
	<tbody>
      ...
      ?>
          <tr style="cursor: pointer;" onclick="get_data_invoice('<?=$invoice->trx_no?>','<?=$invoice->total_amt?>')">
    		<td><?=$no++?></td>
	...
	<tbody>
      ...
      if (!empty($get_data)) {
          ...
          foreach ($get_data as $ap) {
            ...
            if(count($check_data) == 0){
      ?>
          <tr style="cursor: pointer;" onclick="get_data_invoice('<?=$ap->trx_no?>','<?=$ap->base_amt?>')">
    		<td><?=$no++?></td>
    		<td><?=$ap->trx_no?></td>
          ...
          foreach ($get_data as $deposit) { 
            ...
            if(count($check_data) == 0){
                ...
                if($deposit->type == 'C')
                    ...
                    $type = 'Driver';
      ?>
          <tr style="cursor: pointer;" onclick="get_data_invoice('<?=$deposit->deposit_number?>','<?=$deposit->amount?>')">
    		<td><?=$no++?></td>
	...
	<tbody>
      ...
      if (!empty($get_data)) {
          ...
          foreach ($get_data as $commission) { 
            ...
            if(count($check_data) == 0){
      ?>
          <tr style="cursor: pointer;" onclick="get_data_invoice('<?=$commission->commission_no?>','<?=$commission->driver_commission?>')">
    		<td><?=$no++?></td>
	...
	<tbody>
      ...
      if (!empty($get_data)) {
          ...
          foreach ($get_data as $advance) { 
            ...
            if(count($check_data) == 0){
      ?>
          <tr style="cursor: pointer;" onclick="get_data_invoice('<?=$advance->advance_number?>','<?=$advance->advance_total?>')">
    		<td><?=$no++?></td>
            ...
            if(count($check_data) == 0){
                if($payment_type == 'P'){
                    ...
                    */
                    $paid = $reimburse->paid_total * -1;
      ?>
                  <tr style="cursor: pointer;" onclick="get_data_invoice('<?=$reimburse->reimburse_number?>','<?=$paid?>')">
            		<td><?=$no++?></td>
