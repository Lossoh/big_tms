...
<title><?php echo lang('cash_bank_payment'); ?></title>
</head>
...
<div id="header">
    <h2><?php echo lang('cash_bank_payment'); ?></h2>
...
<table width="100%" border="1" align="center" cellpadding="1" cellspacing="0">
	<tr >    
		<th width="5%">No</th>
		<th width="15%">Payment/Receive No</th>
		<th width="15%">Payment/Receive Date</th>
        <th width="8%">Type</th>
		<th>Cash/Bank</th>
		<th>Remark</th>
        <th width="15%">Payment To/Receive From</th>
    ...
    if(!empty($cash_banks)){
    	foreach($cash_banks as $val){
            ...
            if($val->debtor_creditor_type == 'D'){
                $get_nama = $this->cash_bank_payment_model->get_by_id_table('sa_debtor',$val->debtor_creditor_rowID);
                $nama_pay_to = $get_nama->debtor_name == '' ? '-' : $get_nama->debtor_name;
            }
            else if($val->debtor_creditor_type == 'C'){
                $get_nama = $this->cash_bank_payment_model->get_by_id_table('sa_creditor',$val->debtor_creditor_rowID);
