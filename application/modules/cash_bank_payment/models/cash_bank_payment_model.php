...
class cash_bank_payment_model extends CI_Model
{
    ...
    function simpan_header_release($sa_spec_prefix,$alloc_code,$trx_no,$dataPost,$detGiro=array())
    {
        $get_data = $this->get_data_not_release_by_row_id($detGiro['rowID_release']);
        ...
        $data = array(
            ...
            'trx_no' => $trx_no,
            'trx_date' => $alloc_date,
            'payment_type' => $dataPost['val_cb_payment_type'],
            'transaction_type' => $get_data->transaction_type,
            'coa_rowID' => $get_data->coa_rowID,
            'descs' => $cancel.'RELEASE CHEQUE/GIRO FROM '.$dataPost['val_cb_payment_no_release'],
            'trx_amt' => $get_data->cg_amt,
            'fund_trf_coa_rowID' => 0,//(isset($dataPost['cb_pay_to'])) ? $dataPost['cb_pay_to'] : 0,
            'debtor_creditor_rowID'=>$get_data->debtor_creditor_rowID,
            'debtor_creditor_type'=>$get_data->debtor_creditor_type,
            'manual_debtor_creditor'=>$get_data->manual_debtor_creditor,
            'manual_debtor_creditor_type'=>$get_data->manual_debtor_creditor_type,
            'branch'=>$this->session->userdata('dep_rowID'),
            'user_created' => $this->session->userdata('user_id'),
            'date_created' => $alloc_date,
            'time_created' => date('H:i:s'));
    ...
    function simpanDetailGiroRelease($sa_spec_prefix, $alloc_code,$i,$trx_no,$dataPost,$detGiro=array()){
        $get_data = $this->get_by_id_table('cb_trx_cg',$detGiro['rowID_release']);
        
        $cb_payment_date = date('Y-m-d',strtotime($detGiro['cb_release_date']));
        $year = date('Y',strtotime($cb_payment_date));
        $month = date('m',strtotime($cb_payment_date));
        ...
        $data = array(
            ...
            'cash_bank' => $get_data->cash_bank,
            'cg_no' => $get_data->cg_no,
            'cg_date' => $get_data->cg_date,            
            'cg_amt' => $get_data->cg_amt,
            'status' => $detGiro['release_status'],
            'user_created' => $this->session->userdata('user_id'),
            'date_created' => $cb_payment_date,
            'time_created' => date('H:i:s')
        );
    ...
    function updateDetailGiro($detGiro=array(),$trx_no){
        $get_data = $this->get_by_id_table('cb_trx_cg',$detGiro['rowID_release']);
        
        $data_updated = array(
            'status' => $detGiro['release_status'],
            'reference_release_no' => $trx_no,
			'user_modified'     =>$this->session->userdata('user_rowID'),
			'date_modified'     =>date('Y-m-d'),
			'time_modified'     =>date('H:i:s')
        );
        $this->db->where('rowID',$detGiro['rowID_release']);
        $result=$this->db->update('cb_trx_cg', $data_updated);
    ...
    function simpanDetailPayment($sa_spec_prefix,$alloc_code,$x,$trx_no,$dataPost,$detPay=array()){
        ...
        if($dataPost['payment_type'] == 'P'){
            if($dataPost['cb_trx_type'] == 'reimburse'){
    ...
    function get_detail_by_reference($cb_prefix,$cb_year,$cb_month,$cb_code)
    {
        $this->db->select('*');
    ...
    function get_all_advance_by_debtor_id($id)
    {
        ...
        $this->db->where('a.deleted', 0);
        $this->db->where("a.debtor_rowID",$id);
        $this->db->order_by('a.advance_number','desc');
    ...
    function get_all_reimburse_list_by_debtor_id($id)
    {
        $this->db->select("DISTINCT a.*, b.advance_name",false);
		$this->db->from('tr_reimburse_trx_hdr as a');
		$this->db->join('sa_advance_category as b', 'b.rowID = a.advance_type_rowID');
		$this->db->join('tr_reimburse_trx_adv_dtl as c', 'c.reimburse_number = a.reimburse_number');
		$this->db->join('tr_advance_trx_hdr as d', 'd.advance_number = c.advance_number');
        $this->db->where('a.deleted', 0);
        $this->db->where('c.deleted', 0);
        $this->db->where('d.deleted', 0);
        $this->db->where("d.debtor_rowID",$id);
        $this->db->order_by('a.reimburse_number','desc');
    ...
    function get_all_record_data($start_date,$end_date){
        ...
        $this->db->select("a.*,
                CONCAT(c.acc_cd,' - ',c.acc_name) AS cash_bank,
                concat(d.acc_cd,' - ',d.acc_name) as pay_to_cd ", false);
        $this->db->from('cb_trx_hdr as a');
        $this->db->join('gl_coa as c', 'c.rowID = a.coa_rowID','left');
        $this->db->join('gl_coa as d', 'd.rowID = a.fund_trf_coa_rowID',
            'left');
        $this->db->where('a.deleted', 0);
    ...
    {
        $this->db->select('*');
        $this->db->from('tr_cost_trx');
        $this->db->where('trx_no', $trx_no);
        $this->db->where('row_no > ', 1);
        $this->db->where('deleted', 0);
        $query = $this->db->get();
        return $query->result();
    }
    
    function get_data_not_release_by_row_id($row_id)
    {
        $this->db->select('a.rowID, a.cg_amt, b.*');
