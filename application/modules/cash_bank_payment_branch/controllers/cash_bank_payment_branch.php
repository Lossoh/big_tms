...
class cash_bank_payment_branch extends MX_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('cash_bank_payment_branch_model');
    ...
    function pdf()
    {
        ...
        $data['cash_banks'] = $this->cash_bank_payment_branch_model->get_all_record_data($start_date,$end_date);
        
        $html = $this->load->view('cash_bank_payment_branch_pdf', $data, true);
        $this->pdf_generator->generate($html, 'cash bank payment (branch)', $orientation = 'Portrait'); //Portrait
    }

    function excel()
    {
        header("Content-type: application/octet-stream");
        header("Content-Disposition: attachment; filename=cash_bank_payment_branch.xls");
        ...
        $data['cash_banks'] = $this->cash_bank_payment_branch_model->get_all_record_data($start_date,$end_date);
        $this->load->view("cash_bank_payment_branch_pdf", $data);

    }

    function print_cash_bank_payment_branch()
    {
        ...
        $get_data = $this->cash_bank_payment_branch_model->get_cb_by_id($cb_prefix,$cb_year,$cb_month,$cb_code);
        if($get_data->debtor_creditor_type == 'C'){
            $get_data = $this->cash_bank_payment_branch_model->get_cb_customer_by_id($cb_prefix,$cb_year,$cb_month,$cb_code);
		...
		$params['module'] = 'Cash Bank Payment (Branch)';
        ...
        $html = $this->load->view('cash_bank_payment_branch_print_pdf', $data, true);
    ...
    function index()
    {
        ...
        $this->template->title('Cash Bank Payment (Branch)' . ' - ' . $this->config->item('website_name') .
            ' ' . $this->config->item('version'));
        $data['page'] = 'Cash Bank Payment (Branch)';
        $this->session->set_userdata('page_header', 'transaction');
        $this->session->set_userdata('page_detail', 'cash_bank_payment_branch');
        ...
        // $data['cash_bank_payment_branchs'] = $this->cash_bank_payment_branch_model->get_all_record_data($start_date,$end_date);
        $data['coas'] = $this->cash_bank_payment_branch_model->get_account();
        
        $this->template->set_layout('users')->build('cash_bank_payment_branch', isset($data) ?
            $data : null);
    }
    ...
    function set_filter(){
       ...
       redirect(base_url().'cash_bank_payment_branch');
    }
    
    function get_data_debtor(){
        $employee_type = $this->input->post('employee_type');
        
        $debtor = $this->cash_bank_payment_branch_model->get_all_records('sa_debtor', $array =
            array('rowID >' => 0, 'deleted' => 0, 'type' => $employee_type), $join_table = '', $join_criteria = '', 'debtor_cd', 'asc');
    ...
    function get_debtor(){
        ...
        || $trx_type == 'advance' || $trx_type == 'reimburse' || $trx_type == 'general'){
            ...
            if($trx_type == 'cash_advance' || $trx_type == 'commission' || $trx_type == 'deposit' || $trx_type == 'advance' 
                ...
                $sql = "SELECT * FROM sa_debtor WHERE deleted = 0 AND type = 'D' OR type = 'E' ORDER BY debtor_name";
                $des = $this->db->query($sql)->result();
            }
            else if($trx_type == "ar"){
                $sql = "SELECT * FROM sa_debtor WHERE deleted = 0 AND type = 'C' ORDER BY debtor_name";
                $des = $this->db->query($sql)->result();
            }            
            ...
            if (count($des) > 0) {
                ...
                foreach ($des as $rs) {
                    ...
                    $option .= "<option value='".$rs->rowID."'>".$rs->debtor_cd.' - '.$rs->debtor_name." ".$type."</option>";
                }
            }
            else{
            ...
            $des = $this->cash_bank_payment_branch_model->get_all_records($table = 'sa_creditor', $array =
    ...
    function get_data_advance_invoice(){
        ...
        if($tipe == 'cash_advance'){
            $get_data = $this->cash_bank_payment_branch_model->get_all_advance_list_by_debtor_id($id);
        }
        else if($tipe == "invoice"){
            $get_data = $this->cash_bank_payment_branch_model->get_all_invoice_list_by_debtor_id($id);
        }
        else if($tipe == "ap"){
            $get_data = $this->cash_bank_payment_branch_model->get_all_ap_list_by_debtor_id($id);
        }
        else if($tipe == "deposit"){
            $get_data = $this->cash_bank_payment_branch_model->get_all_deposit_list_by_debtor_id($id);
        }
        else if($tipe == 'commission'){
            $get_data = $this->cash_bank_payment_branch_model->get_all_commission_list_by_debtor_id($id);
        }
        else{
            $get_data = array();
        }
    ...
    function get_data_cash_bank()
    {
        ...
        $cb_month  = $_GET['month'];
        $cb_code   = $_GET['code'];
        $hasil = $this->cash_bank_payment_branch_model->get_by_id($cb_prefix,$cb_year,$cb_month,$cb_code);
        Header('Content-Type: application/json; charset=UTF8');
    ...
    function simpan_cash_bank_payment()
    {
        ...
        if (empty($dataPost['cb_payment_no'])) {
            ...
            if($this->session->userdata('page_detail') == 'cash_bank_payment_branch'){
                ...
                $get_data_header = $this->cash_bank_payment_branch_model->get_data_header($dataPost['cb_payment_no']);
        ...
        if($cek_string !== FALSE){
            $check_data_exist = $this->cash_bank_payment_branch_model->get_data_header_by_type_driver('uang_makan',$dataPost['debtor_creditor'],date('Y-m-d',strtotime($alloc_date)));
        ...
        if($cek_string !== FALSE){
            $check_data_exist = $this->cash_bank_payment_branch_model->get_data_header_by_type_driver('stand_by',$dataPost['debtor_creditor'],date('Y-m-d',strtotime($alloc_date)));
            if(count($check_data_exist) > 0){
        ...
        foreach ($dataPost['detailPay'] as $detPay) {
            if($dataPost['cb_trx_type'] == 'cash_advance'){
                $check_data_ca = $this->cash_bank_payment_branch_model->get_data_detail_by_advance_invoice_type_no($dataPost['payment_type'],$dataPost['cb_trx_type'],$detPay['advance_invoice_no']);
        ...
        if($process == true){
            ...
            $check_trx_no = $this->cash_bank_payment_branch_model->get_data_header($trx_no);
            ...
            if (empty($dataPost['cb_payment_no'])) {
                // simpan header
                $result = $this->cash_bank_payment_branch_model->simpan_header($sa_spec_prefix, $alloc_code,$trx_no,$dataPost);
                $id_header = $result;
                $edit = 0;
                ...
                $result = $this->cash_bank_payment_branch_model->edit_header($dataPost);
                $edit = 1;
    
                $result = $this->cash_bank_payment_branch_model->deleteDetailChegue($sa_spec_prefix,$alloc_date_year,$alloc_date_month,$alloc_code);
                $result = $this->cash_bank_payment_branch_model->deleteDetailPay($sa_spec_prefix,$alloc_date_year,$alloc_date_month,$alloc_code);
            }
            ...
            if (!empty($dataPost['detailgiro'])) {
                ...
                foreach ($dataPost['detailgiro'] as $detGiro) {
                    $i++;
                    $this->cash_bank_payment_branch_model->simpanDetailGiro($sa_spec_prefix, $alloc_code,$i,$trx_no,$dataPost,$detGiro);
                }
            }
            ...
            if (!empty($dataPost['detailPay'])) {
                ...
                *
                $get_cb_alloc = $this->cash_bank_payment_branch_model->get_data_alloc_by_alloc_no('cb_cash_adv_alloc',$trx_no);
                $jumlah_cb_alloc = count($get_cb_alloc);
                $get_dep_alloc = $this->cash_bank_payment_branch_model->get_data_alloc_by_alloc_no('tr_deposit_trx_alloc',$trx_no);
                $jumlah_dep_alloc = count($get_dep_alloc);
                $get_ar_alloc = $this->cash_bank_payment_branch_model->get_data_alloc_by_alloc_no('ar_trx_hdr_alloc',$trx_no);
                $jumlah_ar_alloc = count($get_ar_alloc);
                $get_ap_alloc = $this->cash_bank_payment_branch_model->get_data_alloc_by_alloc_no('ap_trx_hdr_alloc',$trx_no);
                $jumlah_ap_alloc = count($get_ap_alloc);
                $get_commission_alloc = $this->cash_bank_payment_branch_model->get_data_alloc_by_alloc_no('tr_commission_trx_alloc',$trx_no);
                ...
                */
                $x = 0;
                $del_cb_alloc = $this->cash_bank_payment_branch_model->delete_data_alloc_by_alloc_no('cb_cash_adv_alloc',$trx_no);
                $del_dep_alloc = $this->cash_bank_payment_branch_model->delete_data_alloc_by_alloc_no('tr_deposit_trx_alloc',$trx_no);
                $del_ar_alloc = $this->cash_bank_payment_branch_model->delete_data_alloc_by_alloc_no('ar_trx_hdr_alloc',$trx_no);
                $del_ap_alloc = $this->cash_bank_payment_branch_model->delete_data_alloc_by_alloc_no('ap_trx_hdr_alloc',$trx_no);
                $del_commission_alloc = $this->cash_bank_payment_branch_model->delete_data_alloc_by_alloc_no('tr_commission_trx_alloc',$trx_no);
                            
                foreach ($dataPost['detailPay'] as $detPay) {
                    $x++;
                    $this->cash_bank_payment_branch_model->simpanDetailPayment($sa_spec_prefix, $alloc_code, $x,
                    ...
                    */
                    if($dataPost['cb_trx_type'] == 'cash_advance'){
                        if($edit == 1){
                            ...
                            $this->cash_bank_payment_branch_model->edit_cash_advance_update($dataPost,$detPay);                        
                        }
    
                        // Update cash advance -> advance_balance dan pay_over_allocation
                        $this->cash_bank_payment_branch_model->edit_cash_advance($dataPost,$detPay);
                        
                        // Update cash advance alloc
                        $this->cash_bank_payment_branch_model->simpan_realization_hdr($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                    }
                    else if($dataPost['cb_trx_type'] == 'deposit'){
                        $this->cash_bank_payment_branch_model->simpan_deposit_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                    }
                    else if($dataPost['cb_trx_type'] == 'ar'){
                        $this->cash_bank_payment_branch_model->simpan_ar_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                    }
                    else if($dataPost['cb_trx_type'] == 'ap'){
                        $this->cash_bank_payment_branch_model->simpan_ap_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                    }
                    else if($dataPost['cb_trx_type'] == 'commission'){
                        $this->cash_bank_payment_branch_model->simpan_commission_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                    }
                    ...
                    *
                    if($edit == 0){
                        ...
                        if($dataPost['cb_trx_type'] == 'cash_advance'){
                            $this->cash_bank_payment_branch_model->edit_realization_hdr($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                            ...
                            $this->cash_bank_payment_branch_model->edit_deposit_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                            ...
                            $this->cash_bank_payment_branch_model->edit_ar_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                            ...
                            $this->cash_bank_payment_branch_model->edit_ap_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
                            ...
                            $this->cash_bank_payment_branch_model->edit_commission_alloc($sa_spec_prefix, $alloc_code, $x,
                            $trx_no, $dataPost, $detPay);
    ...
    function delete_data() // Tidak dipakai
    {
        error_reporting(E_ALL);
        ...
        $get_data = $this->cash_bank_payment_branch_model->get_detail_by_reference($cb_prefix,$cb_year,$cb_month,$cb_code);
        
        $get_data_cb = $this->cash_bank_payment_branch_model->get_data_by_id($cb_prefix,$cb_year,$cb_month,$cb_code);
        ...
        $data = $this->cash_bank_payment_branch_model->delete_data_header($cb_prefix,$cb_year,$cb_month,$cb_code);
        $data = $this->cash_bank_payment_branch_model->delete_data_detail_giro($cb_prefix,$cb_year,$cb_month,$cb_code);
        $data = $this->cash_bank_payment_branch_model->delete_data_detail_Pay($cb_prefix,$cb_year,$cb_month,$cb_code);
        $data = $this->cash_bank_payment_branch_model->delete_data_realization($cb_prefix,$cb_year,$cb_month,$cb_code);
    ...
    function showDetailGiro(){
        ...
        $detGiroData = $this->db->get_where('cb_trx_cg', array('cb_trx_hdr_prefix' =>
                $cb_prefix, 'cb_trx_hdr_year' => $cb_year,'cb_trx_hdr_month' =>$cb_month,'cb_trx_hdr_code'  =>$cb_code));
    ...
    function fetch_data() { 
        if ($this->input->is_ajax_request()) {
            ...
            if (!empty($dt['columns'][9]['search']['value'])) {
                ...
                $sWhere .= " sa_users.dep_rowID = " . $this->session->userdata('dep_rowID') . " AND cb_trx_hdr.deleted = 0  ". $str_between . " AND (cb_trx_hdr.transaction_type = 'cash_advance' OR cb_trx_hdr.transaction_type = 'general' OR cb_trx_hdr.transaction_type = 'uang_makan' OR cb_trx_hdr.transaction_type = 'stand_by') ";
            }
            ...
            if (isset($sSearchVal) && $sSearchVal != '') {
                ...
                $sWhere .= ") AND sa_users.dep_rowID = " . $this->session->userdata('dep_rowID') . " AND cb_trx_hdr.deleted = 0  ". $str_between . " AND (cb_trx_hdr.transaction_type = 'cash_advance' OR cb_trx_hdr.transaction_type = 'general' OR cb_trx_hdr.transaction_type = 'uang_makan' OR cb_trx_hdr.transaction_type = 'stand_by') ";
            }

            /** Total Data Set Length * */
            $sQuery = "SELECT * FROM " . $dt['table'] . " LEFT JOIN gl_coa as cash_bank ON cash_bank.rowID = cb_trx_hdr.coa_rowID LEFT JOIN gl_coa as pay_to_cd ON pay_to_cd.rowID = cb_trx_hdr.fund_trf_coa_rowID LEFT JOIN sa_debtor ON sa_debtor.rowID = cb_trx_hdr.debtor_creditor_rowID LEFT JOIN sa_creditor ON sa_creditor.rowID = cb_trx_hdr.debtor_creditor_rowID LEFT JOIN sa_users  ON sa_users.rowID = cb_trx_hdr.user_created WHERE sa_users.dep_rowID = " . $this->session->userdata('dep_rowID') . " AND cb_trx_hdr.deleted = 0  ". $str_between . " AND (cb_trx_hdr.transaction_type = 'cash_advance' OR cb_trx_hdr.transaction_type = 'general' OR cb_trx_hdr.transaction_type = 'uang_makan' OR cb_trx_hdr.transaction_type = 'stand_by') ";
            ...
            if ($sWhere != "") {
                ...
                $where = " WHERE sa_users.dep_rowID = " . $this->session->userdata('dep_rowID') . " AND cb_trx_hdr.deleted = 0  ". $str_between . " AND (cb_trx_hdr.transaction_type = 'cash_advance' OR cb_trx_hdr.transaction_type = 'general' OR cb_trx_hdr.transaction_type = 'uang_makan' OR cb_trx_hdr.transaction_type = 'stand_by') ";
            }
            ...
            if (!empty($rResult)) {
                ...
                foreach ($rResult->result_array() as $aRow) {
                    ...
                    $get_data = $this->cash_bank_payment_branch_model->get_data_not_release($aRow['trx_no']);
                    $get_data_released = $this->cash_bank_payment_branch_model->get_data_released($aRow['trx_no']);
                    ...
                    $get_data_release = $this->cash_bank_payment_branch_model->get_data_release_reference($aRow['trx_no']);
                    ...
                    if($this->get_user_access('PrintLimited') == 1 || $this->get_user_access('PrintUnlimited') == 1 || 
                        ...
                        if($this->get_user_access('PrintLimited') == 1){
                            if($this->user_profile->get_log_limited_printed($aRow['trx_no'],'Cash Bank Payment (Branch)') == 0){
                                if(count($get_data_release) > 0){
                                    ...
                                    $dropdown_option .= '<li><a  href="javascript:void()" title="' . lang('print_option') . '" onclick="print_cash_bank_payment_branch(\'' . $aRow['prefix'] . '\',\'' . $aRow['year'] . '\',\'' .$aRow['month'] . '\',\'' . $aRow['code'] . '\')"><i class="fa fa-print"></i> ' . lang('print_option') . '</a></li>';                                       
                                }
                            }
                        } else{
                            if(count($get_data_release) > 0){
    ...
    function get_user_access($field){
        ...
        $this->db->where('Link_Menu', 'cash_bank_payment_branch');
