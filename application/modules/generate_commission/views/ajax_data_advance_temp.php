<table class="table table-responsive table-striped table-condensed" width="100%">
    ...
    <tbody>
        ...
        $i=1;
        $total_payment = 0;        
        if(!empty($get_data_advance)){
            echo '<input type="hidden" name="jumlah_advance" id="jumlah_advance" value="'.count($get_data_advance).'" />';
        ...
        ?>
        		<tr id="baris_<?=$i?>">								
        			<td align="center">
                        <input type="checkbox" name="chk_loan[]" id="chk_loan_<?=$i?>" onclick="check_loan(<?=$i?>)" <?=$row->amount_payment > 0 ? 'checked=""' : '' ?> />
                        <input type="hidden" name="advance_no[]" id="advance_no_<?=$i?>" value="<?=$row->advance_no?>" />
                        <input type="hidden" name="advance_balance[]" id="advance_balance_<?=$i?>" value="<?=$row->amount_loan?>" />
                    </td>
        			...
        			<td align="right"><?= number_format($row->amount_loan,0,',','.')?></td>
        			<td>
                        <div class="input-group">
                            <span class="input-group-addon" id="basic-addon1">Rp</span>
                            <input type="text" class="form-control input-sm text-right currency" name="jumlah_loan[]" id="jumlah_loan_<?=$i?>" onkeyup="calculation_loan(<?=$i?>)" value="<?= number_format($row->amount_payment,0,',','.')?>" readonly="" />
                        </div>	
                    </td>
        		</tr>
        <?php 
                $total_payment += $row->amount_payment;
                $i++;
            }
            echo '<script> 
                    $("#nilai_loan").html("'.number_format($total_payment,0,',','.').'");
                    $("#btnGetLoan").show(); </script>';
        }
        else{
            echo '<tr>
