...
if(count($drivers) > 0){
    $no = 1;
    $jumlah_driver = count($drivers);
    foreach($drivers as $row){
        $deliveries = $this->generate_commission_model->get_all_do_by_comm_debtor_id($comm_no,$row->rowID,$get_comm->until_date);
        if (!empty($deliveries)) {
            $driver = $this->generate_commission_model->get_debtor_by_debtor_id($row->rowID);
            $commission = $this->generate_commission_model->get_all_commission_by_comm_id($get_comm->rowID);
            $cash_advances = $this->generate_commission_model->get_cash_advance_by_comm_debtor_id($comm_no,$row->rowID);
            $cash_advance_loans = $this->generate_commission_model->get_data_advance_loan_by_debtor_id($row->rowID);
