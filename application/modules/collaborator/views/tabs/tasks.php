...
if ($task->auto_progress == 'FALSE') { ?>
	...
	<a href="<?=base_url()?>collaborator/tasks/pilot/on/<?=$task->t_id?>/<?=$task->project*8600?>"  data-original-title="<?=lang('auto_progress_on')?>" class="btn btn-xs green" data-toggle="tooltip" data-placement="right"> <i class="fa fa-rocket text-white"></i></a>
<?php }else{ ?>
...
if ($task->timer_status == 'On') { ?>
 <a href="<?=base_url()?>collaborator/tasks/tracking/off/<?=$task->project?>/<?=$task->t_id?>" class="btn red btn-xs active"> <i class="fa fa-clock-o text-muted text"></i> <i class="fa fa-clock-o text-white text-active"></i> <?=lang('stop_timer')?> </a> 
