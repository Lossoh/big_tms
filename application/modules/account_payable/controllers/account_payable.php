...
class Account_payable extends MX_Controller
{

    function __construct(){
        parent::__construct();
        
        $this->load->model('account_payable_model');
    ...
    function index()
    {
        ...
        $this->template->title(lang('account_payables') . ' - ' . $this->config->item('website_name') .
            ' ' . $this->config->item('version'));
        $data['page'] = lang('account_payables');
        $this->session->set_userdata('page_header', 'transaction');
        $this->session->set_userdata('page_detail', 'account_payable');
        ...
        // $data['aps'] = $this->account_payable_model->get_all_record_data($start_date,$end_date);
        $data['cash_advance_jo'] =$this->account_payable_model->get_data_cash_advance_jo();
        ...
        $this->template->set_layout('users')->build('account_payables', isset($data) ? $data : null);
    }
    ...
    function set_filter(){
       ...
       redirect(base_url().'account_payable');
    }
    ...
    function get_data_header(){
        ...
        $data = $this->account_payable_model->get_data_header($trx_no);
        Header('Content-Type: application/json; charset=UTF8');
    ...
    function get_data_detail(){
        ...
        $data = $this->account_payable_model->get_data_detail($trx_no);
        Header('Content-Type: application/json; charset=UTF8');
        echo json_encode($data);
        exit;
    ...
    function get_data_jo(){
        ...
        $data = $this->account_payable_model->get_data_jo($jo_no);
        Header('Content-Type: application/json; charset=UTF8');
        echo json_encode($data);
        exit;
    ...
    function check_data_do(){
        ...
        $data = $this->account_payable_model->get_data_do($do_no);
        Header('Content-Type: application/json; charset=UTF8');
        echo json_encode($data);
        exit;
    ...
    function get_data_supplier(){
        ...
        $creditor = $this->account_payable_model->get_all_records('sa_creditor', $array =
    ...
    function print_ap($trx_no){
        $get_data = $this->account_payable_model->get_detail_header($trx_no);
		...
		$params['module'] = 'Account Payable';
		$params['module_field_id'] = $get_data->rowID;
		$params['activity'] = ucfirst('Print a Account Payable No. '.$trx_no);
		$params['icon'] = 'fa-print';
		modules::run('activitylog/log',$params); //log activity	
        
        $html = $this->load->view('account_payable_pdf', $data, true);
        
        $this->pdf_generator->generate($html, 'Account Payable pdf',$orientation='Portrait');    
    }
    ...
    function save_ap(){
        ...
        */
        ...
        $sa_spec = $this->db->get_where('sa_spec', array('deleted' => 0, 'rowID' => 1))->
            row_array();
        $sa_spec_prefix = $sa_spec['ap_inv'];
        
        if($trx_no != '' || $row_id != '' || $code != ''){
            $this->account_payable_model->delete_ap_hdr($trx_no); 
            $this->account_payable_model->delete_ap_trx_dtl_do($trx_no); 
            $this->account_payable_model->delete_gl_header($trx_no); 
            $this->account_payable_model->delete_gl_detail($trx_no); 
        ...
        $result = $this->account_payable_model->simpan_ap_hdr($sa_spec_prefix, $alloc_code, $alloc_no, $dataPost); 

        if ($error == false)
        {    
            $result = $this->account_payable_model->save_gl_header($sa_spec_prefix_gl, $new_gl_coa_code, $gl_coa_no, $alloc_no, $alloc_date, $dataPost);          
            if ($result){
                
                $get_creditor = $this->account_payable_model->get_by_id('sa_creditor', $dataPost['creditor_id']);
                ...
                $result = $this->account_payable_model->save_gl_detail_debit($sa_spec_prefix_gl, $new_gl_coa_code, $gl_coa_no, 0, $dataPost['remark'], $amount_dc, 'C', $dataPost['creditor_id'], $alloc_no, 
                                                                            $alloc_date, $dataPost);         
                $tax_amt = str_replace('.','',$dataPost['tax_amt']);
                if($tax_amt > 0){
                    $result = $this->account_payable_model->save_gl_detail_debit($sa_spec_prefix_gl, $new_gl_coa_code, $gl_coa_no, $coa_ppn_rowID, $dataPost['remark'], $tax_amt, 'C', $dataPost['creditor_id'], $alloc_no, 
                                                                            $alloc_date, $dataPost);
                }
                
                if ($result){
                    $result = $this->account_payable_model->save_gl_detail_credit($sa_spec_prefix_gl, $new_gl_coa_code, $gl_coa_no, $coa_rowID, $dataPost['remark'], $total_amt, 'C', $dataPost['creditor_id'], $alloc_no, 
                                                                            $alloc_date, $dataPost);          
                    if ($result){
                        if (!empty($dataPost['detailDO']))
                        {
                            ...
                            {
                                $result = $this->account_payable_model->simpan_data_detail_do($sa_spec_prefix, $alloc_code, $alloc_no,
                                    $dataPost, $detDO);
                                if (!$result)
                                {
                                    $error = true;
        ...
        if ($status === false || $error == true)
        {
            ...
            $params['user_rowID'] = $this->tank_auth->get_user_id();
            $params['module'] = 'ERROR ROLLBACK Account Payable';
            $params['module_field_id'] = $alloc_code;
            $params['activity'] = ucfirst('Deleted a Account Payable No. ' . $alloc_no);
        ...
        } else
        {
            ...
            if($trx_no != '' || $row_id != '' || $code != ''){
				...
				$params['activity'] = ucfirst('Update a Account Payable No. '.$trx_no);
				...
				$params['module'] = 'Account Payable';
				$params['module_field_id'] = $alloc_code;
				$params['activity'] = ucfirst('Add a new Account Payable No. '.$alloc_no);
				$params['icon'] = 'fa-plus';
				modules::run('activitylog/log',$params); //log activity	
                
            }        
            $info = lang('account_payable_registered_successfully');
    ...
    function delete_ap(){
        ...
        if ($status === false || $error == true)
        {
            ...
            $params['activity'] = ucfirst('Deleted a Account Payable No. ' . $trx_no);
            $params['icon'] = 'fa-exclamation-triangle';
            modules::run('activitylog/log', $params);
            echo json_encode(array('success' => false, 'msg' => " Failed"));
            exit();
        } else
        {
			...
			$params['module'] = 'Account Payable';
			$params['module_field_id'] = $trx_no;
			$params['activity'] = ucfirst('Delete a Account Payable No. '.$trx_no);
    ...
    function fetch_data() { 
        if ($this->input->is_ajax_request()) {
            ...
            $aColumnTable = array(
                'ap_trx_hdr.rowID', 'ap_trx_hdr.trx_no', 'ap_trx_hdr.trx_date', 'sa_creditor_type.descs', 'sa_creditor.creditor_name', 'ap_trx_hdr.ref_no', 'ap_trx_hdr.base_amt', 'ap_trx_hdr.total_amt', 'sa_creditor.creditor_cd'
            );
            ...
            if (!empty($dt['columns'][8]['search']['value'])) {
                ...
                $sWhere.= ' ap_trx_hdr.deleted = 0 AND ap_trx_hdr.ap_kb_type = "ap" ' . $str_between; 
            }
            ...
            if (isset($sSearchVal) && $sSearchVal != '') {
                ...
                $sWhere .= ') AND ap_trx_hdr.deleted = 0 AND ap_trx_hdr.ap_kb_type = "ap" ' . $str_between;
            } 

            /** Total Data Set Length * */
            $sQuery = 'SELECT * FROM ' . $dt['table'] . ' LEFT JOIN sa_creditor ON sa_creditor.rowID = ap_trx_hdr.creditor_rowID LEFT JOIN sa_creditor_type ON sa_creditor_type.rowID = ap_trx_hdr.ap_type WHERE ap_trx_hdr.deleted = 0 AND ap_trx_hdr.ap_kb_type = "ap" ' . $str_between;
            ...
            if ($sWhere != "") {
                ...
                $where = " WHERE ap_trx_hdr.deleted = 0 AND ap_trx_hdr.ap_kb_type = 'ap' " . $str_between;
            }
                ...
                foreach ($rResult->result_array() as $aRow) {
                    ...
                    if($this->get_user_access('PrintLimited') == 1 || $this->get_user_access('PrintUnlimited') == 1 || $this->get_user_access('PrintOne') == 1 || $this->get_user_access('PrintTwo') == 1){
                        if($this->get_user_access('PrintLimited') == 1){
                            if($this->get_log_limited_printed($aRow['trx_no'],'Account Payable') == 0){
                    ...
                    $check_invoice = $this->account_payable_model->check_invoice($aRow['trx_no']);
                    ...
                    $row['creditor_type'] = ucwords(strtolower($aRow['descs']));
                    $row['creditor'] = strtoupper($aRow['creditor_cd'] . ' - ' . $aRow['creditor_name']);
    ...
    function get_user_access($field){
        ...
        $this->db->where('Link_Menu', 'account_payable');
