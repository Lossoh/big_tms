...
class Verification_document_model extends CI_Model
{
    ...
    function get_data_do_by_id($row_id)
	{
        $this->db->select('a.rowID,a.jo_no,a.do_no,a.komisi_supir,a.komisi_kernet,a.container_row_no,a.container_no,a.count_container,a.container_size,a.deliver_date,a.deliver_weight,
                            a.received_date,a.received_weight,b.jo_type,b.destination_from_rowID,b.destination_to_rowID, c.vehicle_type_rowID, d.debtor_name, e.police_no, f.alloc_amt');
        $this->db->from('tr_do_trx as a');
        ...
        $this->db->join('cb_cash_adv_alloc as f','f.alloc_no = a.trx_no','left');
        $this->db->where('a.deleted', 0);
        $this->db->where('b.deleted', 0);
    ...
    function update_data_by_row_id($row_id,$status){
        ...
        if($status == 1 && $check_verification_document['verified'] == 0){
            $this->db->set('first_comm_driver',$check_verification_document['komisi_supir']);
            $this->db->set('first_comm_co_driver',$check_verification_document['komisi_kernet']);
    ...
    function update_document($dataPost){            
        if($dataPost['jo_verify'] == '0'){
            if($dataPost['jo_type'] == 2){
                $count_container = 0;
                $container_size = '';
                if($dataPost['container_size'] == '20' || $dataPost['container_size'] == '40' || $dataPost['container_size'] == '45'){
                    $count_container = 1;
                    $container_size = $dataPost['container_size'];
                }
                else if($dataPost['container_size'] == '220'){
                    $count_container = 2;
                    $container_size = 20;
                }
                
                $this->db->set('container_no',$dataPost['container_no']);
                $this->db->set('count_container',$count_container);
                $this->db->set('container_size',$container_size);
            }
        }
        
        $this->db->set('container_row_no',$dataPost['container_row_no']);
