...
class Verification_document extends MX_Controller
{
    ...
    function index()
    {
        ...
        //     $str_between = "AND `a`.`date_created` BETWEEN '".date('Y-m-01')."' and '".date("Y-m-t")."'";
        ...
        $sql = "SELECT `a`.`rowID`, `a`.`trx_no`, `a`.`jo_no`, `a`.`do_no`, a.deliver_weight, a.received_weight, a.deliver_date, a.received_date, `a`.`status`, `a`.`deleted`, `a`.`invoice_no`, `a`.`commission_no`, `a`.`komisi_supir`, `a`.`komisi_kernet`, `a`.`date_verified`,
                ...
                WHERE `a`.`deleted` = 0 AND `b`.`deleted` = 0 AND `e`.`deleted` = 0 AND `f`.`deleted` = 0 AND `a`.`status` = 0 ".$str_between." 
    ...
    function get_data_detail(){
        ...
        $hasil = $this->verification_document_model->get_data_do_by_id($row_id);
    ...
    function get_data_jo(){
        ...
        $hasil = $this->verification_document_model->get_data_jo_by_filter($jo_type,$from_id,$to_id);
    ...
    function verify_password()
    {
        ...
        if (!is_null($user = $this->users->get_user_by_id($user_id, true)))
        {
            ...
            $get_user = $this->verification_document_model->get_verify_user();
            ...
            foreach($get_user as $row_user){
                ...
                {   
                    ...
                    if (empty($check_verification_document['rowID'])) {
                        ...
                        $this->verification_document_model->update_data_by_row_id($row_id,$status);
    ...
    function update_document()
    {
        ...
        if (empty($check_verification_document['rowID'])) {
            ...
            $this->verification_document_model->update_document($dataPost);
    ...
    function pdf()
    {
        ...
        $sql = "SELECT `a`.`rowID`, `a`.`trx_no`, `a`.`jo_no`, `a`.`do_no`, a.deliver_weight, a.received_weight, a.deliver_date, a.received_date, `a`.`status`, `a`.`deleted`, `a`.`invoice_no`, `a`.`commission_no`,
                ...
                FROM (`tr_do_trx` as a) 
                    ...
                    LEFT JOIN `sa_destination` as h ON `f`.`destination_to_rowID` = `h`.`rowID`
                    LEFT JOIN `sa_item` as i ON `f`.`item_rowID` = `i`.`rowID`
                WHERE `a`.`deleted` = 0 AND `b`.`deleted` = 0 AND `e`.`deleted` = 0 AND `a`.`status` = 0 ".$str_between." 
    ...
    function detail_do(){
        ...
        $get_data_ca = $this->verification_document_model->get_ca_by_do_no($do_no);
    ...
    function detail_jo(){
        ...
        $get_data_jo = $this->verification_document_model->get_jo_by_jo_no($jo_no);
    ...
    function get_user_access($field){
        ...
        $this->db->where('Link_Menu', 'verification_document');
    ...
    function fetch_data() { 
        if ($this->input->is_ajax_request()) {
            ...
            $dt['id'] = 'rowID';
            $aColumnTable = array(
                'tr_do_trx.rowID', 'sa_debtor.debtor_name', 'tr_do_trx.komisi_supir', 'tr_do_trx.komisi_kernet', 'cb_cash_adv_alloc.doc_sj', 'cb_cash_adv_alloc.doc_st', 'cb_cash_adv_alloc.doc_sm', 'cb_cash_adv_alloc.doc_sr', 'tr_do_trx.do_no', 'tr_do_trx.container_no', 'tr_do_trx.received_date', 'tr_do_trx.received_weight', 'tr_do_trx.jo_no', 'tr_jo_trx_hdr.vessel_name', 'tr_do_trx.date_created', 'tr_do_trx.date_created', 'destination_from.destination_name', 'destination_to.destination_name', 'sa_item.item_name', 'tr_do_trx.status', 'tr_do_trx.trx_no', 'sa_users.username','tr_do_trx.invoice_no'
            );

            $aColumns = array(
              'tr_do_trx.rowID', 'sa_debtor.debtor_name', 'tr_do_trx.komisi_supir', 'tr_do_trx.komisi_kernet', 'cb_cash_adv_alloc.doc_sj', 'cb_cash_adv_alloc.doc_st', 'cb_cash_adv_alloc.doc_sm', 'cb_cash_adv_alloc.doc_sr', 'tr_do_trx.do_no', 'tr_do_trx.container_no', 'tr_do_trx.received_date', 'tr_do_trx.received_weight', 'tr_do_trx.jo_no', 'tr_jo_trx_hdr.vessel_name',  'tr_do_trx.date_created', 'tr_do_trx.date_created', 'destination_from.destination_name as from_name', 'destination_to.destination_name as to_name', 'sa_item.item_name', 'tr_do_trx.status', 'tr_do_trx.trx_no', 'sa_users.username', 'tr_do_trx.invoice_no', 'tr_do_trx.date_created', 'tr_do_trx.date_created'          
            );
            ...
            if (!empty($dt['columns'][14]['search']['value'])) {
                ...
                $sWhere.= ' tr_do_trx.deleted = 0 AND cb_cash_adv.deleted = 0 AND cb_cash_adv_alloc.deleted = 0 AND tr_jo_trx_hdr.deleted = 0 AND tr_do_trx.status = 0 ' . $str_between; 
            }
            ...
            if (isset($sSearchVal) && $sSearchVal != '') {
                ...
                $sWhere .= ') AND tr_do_trx.deleted = 0 AND cb_cash_adv.deleted = 0 AND cb_cash_adv_alloc.deleted = 0 AND tr_jo_trx_hdr.deleted = 0 AND tr_do_trx.status = 0 ' . $str_between;
            }


            /** Total Data Set Length * */
            $sQuery = "SELECT * FROM " . $dt['table'] . " LEFT JOIN cb_cash_adv ON tr_do_trx.trx_no = cb_cash_adv.trx_no LEFT JOIN sa_debtor ON cb_cash_adv.employee_driver_rowID = sa_debtor.rowID LEFT JOIN sa_vehicle ON cb_cash_adv.vehicle_rowID = sa_vehicle.rowID LEFT JOIN cb_cash_adv_alloc ON tr_do_trx.trx_no = cb_cash_adv_alloc.alloc_no LEFT JOIN tr_jo_trx_hdr ON tr_do_trx.jo_no = tr_jo_trx_hdr.jo_no LEFT JOIN sa_destination as destination_from ON tr_jo_trx_hdr.destination_from_rowID = destination_from.rowID LEFT JOIN sa_destination as destination_to ON tr_jo_trx_hdr.destination_to_rowID = destination_to.rowID LEFT JOIN sa_item ON tr_jo_trx_hdr.item_rowID = sa_item.rowID LEFT JOIN sa_users ON tr_do_trx.user_created = sa_users.rowID WHERE tr_do_trx.deleted = 0 AND cb_cash_adv.deleted = 0 AND cb_cash_adv_alloc.deleted = 0 AND tr_jo_trx_hdr.deleted = 0 AND tr_do_trx.status = 0 ". $str_between;
            ...
            if ($sWhere != "") {
                ...
                $where = " WHERE tr_do_trx.deleted = 0 AND cb_cash_adv.deleted = 0 AND cb_cash_adv_alloc.deleted = 0 AND tr_jo_trx_hdr.deleted = 0 AND tr_do_trx.status = 0 " . $str_between;
            }
            ...
            if (!empty($rResult)) {
                ...
                foreach ($rResult->result_array() as $aRow) {
                    ...
                    $row['do_no'] = ($aRow['do_no'] == '') ? '-' : '<a href="javascript:void()" onclick="showDetailDO(\''.$aRow['do_no'].'\')">'. $aRow['do_no'].'</a>';
                    ...
                    if($aRow['status'] == 0){
                        if($this->get_user_access('Verified') == 1){
                            $button_verification =  '<button id="btn_verify_'.$aRow['rowID'].'" class="btn btn-sm btn-success" title="'.lang('verify').'" onclick="verification_document(\''.$aRow['trx_no'].'\',\''. $aRow['rowID'] .'\',\''.$aRow['vessel_name'].'\',\''.$aRow['from_name'].' - '.$aRow['to_name'].'\',\''.$aRow['item_name'].'\')"><i class="fa fa-check"></i></button>';
                        }
                        if($this->get_user_access('Updated') == 1){
                            $button_edit = '<button class="btn btn-sm yellow" title="Edit" id="btn_edit_' . $aRow['rowID'] .'" onclick="edit_document(' . $aRow['rowID'] . ')"><i class="fa fa-edit"></i></button>';
                        }
                    }else{
                        if($this->get_user_access('Verified') == 1){
                            if($aRow['invoice_no'] == ''){
                                $button_unverify = '<button class="btn btn-sm red" title="'.lang('unverify').'" onclick="unverify_document(\''.$aRow['trx_no'].'\',\''.$aRow['rowID'].'\')"><i class="fa fa-times"></i></button>';
                            } 
                        }   
                    }

                    $row['action'] = $button_verification . ' ' . $button_edit . ' ' . $button_unverify;
