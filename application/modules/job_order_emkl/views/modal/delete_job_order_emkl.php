<div class="modal-dialog">
	<div class="modal-content">
		...
		<h4 class="modal-title"><?=lang('delete_option')?> <?=lang('job_order_emkl')?></h4>
		</div>
		<?php
			echo form_open(base_url().'job_order_emkl/delete_job_order_emkl'); 
		?>
		<div class="modal-body">
		<?php
			if (!empty($job_order_emkls)) {
				foreach ($job_order_emkls as $key => $job_order) { ?>
					...
					<p><?=lang('job_order_emkl_no')?><span class="tab"> :</span><b>&nbsp;<?=$job_order['jo_no']?></b></p>
					...
					<p><?=lang('delete_job_order_emkl_warning')?> </p>
		<?php }}?> 
		</div>
		<div class="modal-footer"> <a href="#" class="btn btn-default" data-dismiss="modal"><?=lang('close')?></a>
			<button type="submit" class="btn red"><?=lang('delete')?> <?=lang('job_order_emkl')?></button>
		</form>
	</div>
</div>
<!-- /.modal-content -->
