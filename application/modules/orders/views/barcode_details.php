...
<?php     
	...
	echo form_open(base_url().'orders/verify_barcode', $attributes); ?>
...
<div class="row">						
	<div class="col-xs-6">
		...
		<input type="hidden" name="barcode_id_verify" value="<?=$sjk_detail->barcode_id?>">
		...
		<div class="col-md-4"><?=lang('vessel_name')?></div>
					...
					<div><?php if(!$sjk_detail->user_gateout > 0){?><button type="submit" class="btn btn-sm btn-success"><i class="fa fa-check"></i> <?=lang('verify_barcode')?></button><?php } ?><input class="btn btn-sm yellow" type="button" onclick="window.location.replace('<?=base_url()?>orders/verify_barcode')" value="Back To Verify Barcode" /> </div>
							</form>
					<div></div>
					...
					<?php }} else {?>
							<div class="row inline-fields form-group form-md-line-input">
		<p class="text-danger"> <strong>The page will be refresh left at  <span id="countdown">30</span> seconds.</strong></p>
		<div class="col-md-4"><p class="h5"><strong><?=lang('data_not_found')?></strong></p><input class="btn btn-sm yellow" type="button" onclick="window.location.replace('<?=base_url()?>orders/verify_barcode')" value="Back To Verify Barcode" /> </div>
