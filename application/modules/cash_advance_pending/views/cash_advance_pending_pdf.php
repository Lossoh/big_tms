...
<title><?=lang('cash_advance_pending')?></title>
</head>
...
<br />
<div id="header">
    <span style="font-size: 16px;"><?=strtoupper(lang('cash_advance_pending'))?> REPORT</span>    
</div>
<div id="content">
<table width="100%">
    <tr>
...
<table width="100%" cellpadding="1" cellspacing="0">
	<tr>
        ...
        <th style="border: #000000 solid 1px;" width="10%"><?=lang('cash_advance_no')?></th>
        <th style="border: #000000 solid 1px;">CA <?=lang('date')?></th>
        <th style="border: #000000 solid 1px;"><?=lang('driver')?>/<?=lang('employee')?> Name</th>
        <th style="border: #000000 solid 1px;"><?=lang('fare_trip_code')?></th>
        <th style="border: #000000 solid 1px;">Police No</th>
        <th style="border: #000000 solid 1px;" width="10%">Memo</th>
        <th style="border: #000000 solid 1px;"><?=lang('cash_advance_amt')?></th>
        <th style="border: #000000 solid 1px;"><?=lang('extra_amount')?></th>
        <th style="border: #000000 solid 1px;">Addendum</th>
        <th style="border: #000000 solid 1px;"><?=lang('cash_advance_alloc')?></th>
        <th style="border: #000000 solid 1px;"><?=lang('balance')?></th>
    </tr> 
	<?php 
    $i=0;
    ...
    if (!empty($cash_advance_lists)) {
        foreach ($cash_advance_lists as $cash_advance_list) { 
            ...
            $get_data_memo = $this->cash_advance_pending_model->get_data_memo_by_advance_no($cash_advance_list->advance_no);
            $no = 1;
            $memo = '';
            if(count($get_data_memo) > 0){
                foreach($get_data_memo as $row_memo){
                    $memo .= $no++.'. '.$row_memo->memo_description.'<br/>';
                }
            }
            else{
                $memo = '-';
    ...
    ?>
        	<tr>
                ...
                <td style="border: #000000 solid 1px;"><?=$cash_advance_list->police_no?></td>
                <td style="border: #000000 solid 1px;"><?=$memo?></td>
