<table class="table" width="100%">
    <thead>
    			...
    			<th class="col-md-2" style="text-align: center;">Refund Amount</th>
    </thead>
    <tbody>
        <?php
        ...
        ?>
        		<tr data-rowid="<?php echo $i; ?>">								
        			...
        			<td style="text-align: center;"><?= date("d F Y",strtotime($ca_list->advance_date))?></td>
        			<td style="text-align: right;">Rp <?= number_format($advance_amount,0,',','.')?></td>
        			<td style="text-align: right;">Rp <?= number_format($ca_list->advance_allocation,0,',','.')?></td>
        			<td style="text-align: right;">Rp <?= number_format($ca_list->advance_balance,0,',','.')?><input  type="hidden" id="ca_balance_<?php echo $i; ?>" name="ca_balance[]"  value="<?=$ca_list->advance_balance?>"/></td>	
        			<td style="text-align: right;"><input type="text" data-rowid="<?php echo $i; ?>" class="refund_amount form-control currency" id="refund_amount_<?php echo $i; ?>" name="refund_amount[]" style="font-size:15px;font-weight:600;color: black;text-align: right;" value="<?=number_format($balance,0,',','.')?>" /></td>
        		</tr>
        <?php $i++;
            }
        }
