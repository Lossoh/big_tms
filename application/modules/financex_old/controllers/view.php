...
class View extends MX_Controller {
	...
	function __construct()
	{
		...
		$this->load->model('job_order_model','job_order');
	}
	...
	function update()
	{
		if ($this->input->post()) {
				...
				if ($this->form_validation->run() == FALSE)
				{
						...
						$params['module_field_id'] = $job_order_no;
						$params['activity'] = ucfirst('Updated job order No : '.$job_order_no);
						$params['icon'] = 'fa-edit';
	...
	function delete()
	{
		if ($this->input->post()) {
			...
			$data['job_order_details'] = $this->job_order->get_all_records_update($this->uri->segment(4),$this->uri->segment(5),$this->uri->segment(6)); 
